...
import { hashPlaintextPassword, verifyPasswordHash } from "./password.js";
...
describe("password hash helpers", () => {
  it("hashes plaintext passwords with argon2id and verifies them", async () => {
    ...
    await expect(verifyPasswordHash("correct-horse-battery-staple", passwordHash)).resolves.toBe(
      true,
    );
  });
  ...
  it("rejects non-matching plaintext passwords", async () => {
    ...
    await expect(verifyPasswordHash("wrong-horse-battery-staple", passwordHash)).resolves.toBe(
      false,
    );
  });
  ...
  it("rejects hashes whose numeric parameters exceed the Argon2 API range", async () => {
    await expect(
      verifyPasswordHash(
        "correct-horse-battery-staple",
