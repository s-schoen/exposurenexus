...
Before running applications, follow [Development](docs/development.md) to configure credentials and shared API/worker object storage,
start PostgreSQL and RabbitMQ, and verify one-shot initialization. Then run `pnpm dev:api`, `pnpm dev:worker`, and `pnpm dev:ui`
in separate terminals, waiting for API startup before starting the worker. The complete ingestion handler set activates
consumption automatically; execution remains `pending` in PostgreSQL and completion is visible in worker logs.
...
The [ingestion shell smoke check](docs/deployment.md#ingestion-shell-smoke-check) verifies the handoff using that stack.
