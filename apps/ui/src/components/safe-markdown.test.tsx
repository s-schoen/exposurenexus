...
`}</SafeMarkdown>,
    ...
    expect(container.querySelector("pre code")?.textContent).toContain("GET /admin HTTP/1.1");
...
<style>body { color: red; }</style>`}</SafeMarkdown>,
  ...
  it("blocks unsafe link protocols", () => {
    ...
    expect(screen.getByText("safe").closest("a")?.getAttribute("href")).toBe("https://example.com");
    expect(screen.getByText("mail").closest("a")?.getAttribute("href")).toBe(
      "mailto:security@example.com",
    );
    expect(screen.getByText("relative").closest("a")?.getAttribute("href")).toBe("/findings");
  });
