...
describe("FindingDetailContent", () => {
  ...
  it("renders finding, asset, vulnerability, evidence, and user context", () => {
    ...
    expect(screen.getAllByText("Exposed Admin Endpoint").length).toBeGreaterThan(0);
  ...
  it("assigns an unassigned finding and clears an existing assignee", async () => {
    ...
    const unassigned = render(<FindingDetailContent findingId={mocks.finding.id} />);
    ...
    await waitFor(() => {
      expect(mocks.updateFindingField).toHaveBeenCalledWith(mocks.finding, "assigneeId", null);
    });
  });

  it("sets, changes, and clears due dates from metadata", async () => {
    const undated = render(<FindingDetailContent findingId={mocks.finding.id} />);
  ...
  it("copies evidence and reports success or failure", async () => {
    ...
    const { rerender } = render(<FindingDetailContent findingId={mocks.finding.id} />);
  ...
  it("renders fallbacks for empty evidence and unknown assets", () => {
    ...
    expect(
      screen.getByText("This finding does not include validation notes or scanner output yet."),
    ).toBeTruthy();
    ...
    expect(screen.getByRole<HTMLButtonElement>("button", { name: "Copy evidence" }).disabled).toBe(
      true,
    );
  });
  ...
  it("renders asset owner fallbacks for ownerless assets and unknown users", () => {
    ...
    const ownerless = render(<FindingDetailContent findingId={mocks.finding.id} />);
