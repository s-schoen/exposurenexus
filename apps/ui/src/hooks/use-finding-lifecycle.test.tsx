...
describe("useFindingLifecycle", () => {
  ...
  it("imports finding files and invalidates finding list plus stats", async () => {
    ...
    let importResult: Awaited<ReturnType<typeof result.current.importFindingFile>> | null = null;
    await act(async () => {
  ...
  it("reports finding import failures", async () => {
    ...
    expect(toastErrorMock).toHaveBeenCalledWith(`Failed to upload findings for import: ${error}`);
