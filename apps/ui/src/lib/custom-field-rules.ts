// Form feedback only. Backend capabilities independently enforce these rules.
import {
...
function hasDuplicateAssetCustomFieldValues(values: ReadonlyArray<string>): boolean {
...
export function validateAssetCustomFieldDefinitionRules(
  definition: CreateAssetCustomFieldDefinition | UpdateAssetCustomFieldDefinition,
): Array<AssetCustomFieldRuleViolation> {
  ...
  switch (definition.type) {
    ...
    case AssetCustomFieldType.Select: {
      ...
      const violations: Array<AssetCustomFieldRuleViolation> = [];
