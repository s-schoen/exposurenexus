...
describe("finding api", () => {
  it("lists findings and parses date fields", async () => {
  ...
  it("gets a finding by id and parses date fields", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
  ...
  it("lists nested observations with the observation schema", async () => {
    ...
    const observations = await listFindingObservations(findingId);
  ...
  it("gets and parses finding stats", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        data: findingStats,
      }),
