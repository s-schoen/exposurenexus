...
function StatefulUsersRoute({
  ...
  return <UsersPage search={routeState.search} selected={routeState.selected} />;
}
...
function renderUsersRoute({
  ...
  return render(
    <StatefulUsersRoute initialSearch={initialSearch} initialSelected={initialSelected} />,
  );
}
...
describe("UsersPage", () => {
  ...
  it("opens and closes the selected user preview", async () => {
    ...
    expect(await screen.findByText(`User detail for ${mocks.users[0].id}`)).toBeVisible();
    expect(screen.getByRole("link", { name: /open full page/i })).toHaveAttribute(
      "href",
      `/users/${mocks.users[0].id}`,
    );
