...
describe("user profile repository", () => {
  ...
  it("creates, lists, loads, and updates user profiles", async () => {
    ...
    await expect(repository.create(firstProfile, [builtInRoleIds.viewer])).resolves.toEqual(
      firstProfileWithRoles,
    );
    await expect(repository.create(secondProfile, [builtInRoleIds.editor])).resolves.toEqual(
      secondProfileWithRoles,
    );

    await expect(repository.getByID(firstProfile.id)).resolves.toEqual(firstProfileWithRoles);
    ...
    await expect(repository.getByUsername(updatedFirstProfile.username)).resolves.toEqual(
      updatedFirstProfileWithRoles,
    );

    await expect(repository.getByID(secondProfile.id)).resolves.toEqual(secondProfileWithRoles);
    ...
    expect(profilesAfterUpdate).toEqual(
      expect.arrayContaining([updatedFirstProfileWithRoles, secondProfileWithRoles]),
    );
  });
  ...
  it("deduplicates role assignments on create", async () => {
    ...
    await expect(
      repository.create(firstProfile, [builtInRoleIds.viewer, builtInRoleIds.viewer]),
  ...
  it("rejects duplicate persisted role assignments", async () => {
    const repository = createUserProfileRepository(testDb.db);
    const createdProfile = await repository.create(firstProfile, [builtInRoleIds.viewer]);
