...
>;
type FindingCreatedEvent = DomainEventPayloadBase<"finding.created", { findingId: string }>;
...
describe("EventBus", () => {
  it("emits events to matching listeners in registration order", async () => {
    ...
    eventbus.on("user.*", (receivedEvent) => {
      calls.push(`wildcard:${receivedEvent.subject}:${receivedEvent.data.userId}`);
    });

    await eventbus.emit(event("user.created", { userId: "user-1" }));

    expect(calls).toEqual(["exact:user.created:user-1", "wildcard:user.created:user-1"]);
  });
  ...
  it("handles listener failures and continues emitting", async () => {
    ...
    expect(calls).toEqual(["before-error", "error:user.created:user.created", "after-error"]);
  });
  ...
  it("narrows listener event types from exact and wildcard listener names", () => {
    ...
    eventbus.on("user.authentication.*", (receivedEvent) => {
      expectTypeOf(receivedEvent.subject).toEqualTypeOf<"user.authentication.failure">();
      expectTypeOf(receivedEvent.data.reason).toEqualTypeOf<string>();
    });
