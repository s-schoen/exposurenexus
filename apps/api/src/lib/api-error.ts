...
export function badRequest(message: string, options?: ApiErrorOptions): ApiError {
...
export function forbidden(message: string = "Forbidden", options?: ApiErrorOptions): ApiError {
  return new ApiError(403, message, options);
}

export function notFound(type: string, id: string, options?: ApiErrorOptions): ApiError {
...
function getApplicationErrorStatus(kind: ApplicationErrorKind): ContentfulStatusCode {
  switch (kind) {
    case "validation":
      return 400;
    case "missing":
...
function getErrorStatus(error: ApiError | HTTPException | ApplicationError): ContentfulStatusCode {
  if (isApplicationError(error)) {
...
export function createApiErrorReply(
  ...
  const reply: APIErrorReply = {
    correlationId,
    status,
    error: shouldExposeMessage(error) ? error.message : INTERNAL_SERVER_ERROR_MESSAGE,
  };
...
export function replyError(
  ...
  return c.json(createApiErrorReply(c.get("requestId"), error), getErrorStatus(error));
}
