...
describe("importer", () => {
  ...
  it("dispatches nuclei imports to the nuclei parser", async () => {
    ...
    await expect(importer.parseFindingsFromFile(ctx, FindingSource.Nuclei, file)).resolves.toEqual(
      findings,
    );
  ...
  it("returns an empty result for unsupported import types", async () => {
    ...
    await expect(importer.parseFindingsFromFile(ctx, "unsupported", file)).resolves.toEqual([]);
